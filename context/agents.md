...
As in the concept "fuctoinal core, imperative shell", the domain layer(and partly the application layer) shall be as large as possible. The infra structure layer shall be as small as possible and taking as few "decisions" as possible, while still handling all the I/O, external dependencies, error handling, logging, and mapping to and from external data models such as the DTO defined in the Sikri client crate and the DTOs defined in the lib-schemas crate.
