...
impl IngestCommandService {
    ...
    fn extract_client_reference(&self, command: &Kommando) -> Option<Uuid> {
        match command {
            ...
            Kommando::OpprettInngåendeJournalpost(c) => Some(c.client_reference),
            Kommando::OpprettUtgåendeJournalpost(c) => Some(c.client_reference),
            Kommando::OpprettInterntNotatJournalpost(c) => Some(c.client_reference),
