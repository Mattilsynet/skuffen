...
Renderer authentication uses a Cloud Run metadata-server OIDC identity token with the configured endpoint URL as audience. Logs may include the safe audience label and token acquisition category, but never the token value.
