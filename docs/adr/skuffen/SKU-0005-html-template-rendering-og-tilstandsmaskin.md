...
`Felt::Saksnummer` gjør dokumentet blokkert for rendering til Sikri har gitt saken `saksnummer`. Dette uttrykkes som `BlockedReason`, ikke som implisitt `None` eller entity-lagret ønsket tilstand. HTML-maler uten substitusjonsbehov bruker tom `felter`-liste og kan rendres uten `saksnummer`.
...
R4 [5]: v1 readiness for `Felt::Saksnummer` skal kreve `sak.saksnummer.is_some()`, mens tom `felter`-liste betyr statisk HTML uten substitusjons-prerequisites.
...
R7 [5]: Mismatch mellom deklarerte `felter` og faktiske HTML-tokens er en irrecoverable kontraktsfeil; tom `felter` krever null tokens og setter ellers dokumentet til `FeiletPermanent`.
